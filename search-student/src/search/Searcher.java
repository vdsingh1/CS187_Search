...
public abstract class Searcher<T> {
  ...
  public final boolean isValidSolution(List<T> solution) {
    ...
    for (int i = 1; i < solution.size(); i++) {
      if (!isSuccessor(solution.get(i - 1), solution.get(i))) {
        return false;
      }
    }
    return true;
  ...
  private boolean isSuccessor(T current, T question) {
    List<T> successors = searchProblem.getSuccessors(current);
    return successors.contains(question);
  }
}
